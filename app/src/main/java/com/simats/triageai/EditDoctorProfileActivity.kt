...
import com.simats.triageai.databinding.ActivityEditDoctorProfileBinding
    ...
    private fun loadCurrentProfile() {
        ...
        ApiClient.apiService.getUserProfile(userId).enqueue(object : Callback<UserProfileResponse> {
                ...
                if (response.isSuccessful) {
                    response.body()?.let { profile ->
                        ...
                        profile.profilePhoto?.let { photoPath ->
                            ...
                            // Remove padding when image is loaded
            ...
            override fun onFailure(call: Call<UserProfileResponse>, t: Throwable) {
                Toast.makeText(this@EditDoctorProfileActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        ApiClient.apiService.updateProfile(userId, request).enqueue(object : Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@EditDoctorProfileActivity, "Failed to update profile", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                Toast.makeText(this@EditDoctorProfileActivity, "Network error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun uploadPhoto(imageUri: Uri) {
        ...
        ApiClient.apiService.uploadPhoto(userId, body).enqueue(object : Callback<PhotoUploadResponse> {
            override fun onResponse(call: Call<PhotoUploadResponse>, response: Response<PhotoUploadResponse>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@EditDoctorProfileActivity, "Photo uploaded successfully", Toast.LENGTH_SHORT).show()
                    ...
                    Toast.makeText(this@EditDoctorProfileActivity, "Photo upload failed", Toast.LENGTH_SHORT).show()
                }
            }
